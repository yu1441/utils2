...
public class YDownload extends Thread {
            ...
            if (downloadListener != null) {
                while (!downloadThread.isFinished() && isRun) {
                    if (fileSize == -1) {// 获取不到文件大小时候fileSize=0，进度为0
                        ...
                        double progress = Double.valueOf(downloadThread.getDownloadSize() * 1.0 / fileSize * 100);
    ...
    public interface YDownloadListener {
         ...
         * @param file         下载的文件
         */
        public void response(int downloadSize, int fileSize, double progress, boolean isFail, File file);
    }
