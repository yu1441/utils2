...
public class YDownloadAndroid extends YDownload {
    ...
    public static YDownload Download(String url, String path, String fileName, final YDownloadListener downloadListener) {
        ...
        YDownloadListener listener = new YDownloadListener() {
            @Override
            public void response(final int downloadedSize, final int fileSize, final double progress, final boolean isfail, final File file) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        downloadListener.response(downloadedSize, fileSize, progress, isfail, file);
                    }
                });
            }
        };
