...
public class YnetAndroid extends Ynet {
	...
	protected void sendOther(HttpURLConnection httpURLConnection) throws IOException {
		if ("bitmapMap".equals(flag)) {
			...
			// 文件
			Iterator<Map.Entry<String, Bitmap>> iter = bitmapMap.entrySet().iterator();
			while (iter.hasNext()) {
				...
				strBuf.append("Content-Disposition: form-data; name=\"" + entry.getKey() + "\"; filename=\"" + ("Android" + System.currentTimeMillis() + ".jpg") + "\"\r\n");
	...
	protected void sendBackMsg(final String value, final boolean success) {
		handler.post(new Runnable() {
			...
			public void run() {
				...
				if (showLog)
					System.out.println("请求地址↓：" + (urlType == UrlType.GET ? "\nGet--->" : "\nPost--->") + url + (params == null ? "" : ("\n请求参数：" + params.toString())) + "\n请求结果：" + value);
				if (success) {
	...
	@Override
	protected void sendBackMsg(final byte[] bytes) throws IOException {
	...
	@Override
	protected void sendBackMsg(final InputStream inputStream) throws IOException {
